...
\frame[allowframebreaks]{
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item Prolog formulas that can be used in the declarative part of the agent code
		\end{itemize}
		\vskip 2.0ex
		\item \texttt{<Atom>}: Prolog like atomic formula starting with a capital letter
		\begin{itemize}
			\item Function names that can be used in the imperative part of the agent code
		\end{itemize}
...
\begin{frame}[fragile,allowframebreaks]
	...
	\begin{itemize}
		...
		\item unary operator for plans that must be executed atomically
	\end{itemize}	
...
\begin{frame}[fragile,allowframebreaks]
	...
	goto(X, Y)                // it is an abstract action
...
\begin{frame}[fragile,allowframebreaks]
	...
	They are used to define procedures for:

	\begin{enumerate}
		\item instantiating the execution of abstract actions
		\item handling the reception of messages received from other agents
	...
	A PC rule is applied only if also the belief condition is entailed by the belief base.
...
\frame[allowframebreaks]{
	...
	\item The \emph{exception} are used to apply plan repair rules.
	\end{itemize}
}
...
\frame[allowframebreaks]{
	...
	\bexample
	\begin{verbatim}
	Include: person.2apl    // it implements goto(X,Y)
	\end{verbatim}
	\eexample
}
