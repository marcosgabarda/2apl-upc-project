...
\frame{
	...
	There are 2 distinguished set of programming constructs
	
	\begin{enumerate}
		\item Multi-agent system specification
		\item Individual agent specification
	\end{enumerate}	
}
...
\begin{frame}[allowframebreaks,fragile]
	...
	A multi-agent system is defined by set of:
	...
	\begin{enumerate}
		...
		\begin{itemize}
			\item Any Java class with an internal a state
			\item A set of methods to change the state
...
\frame[allowframebreaks]{
	...
	\begin{itemize}
		\item beliefs	(declarative way)
		\item goals	(declarative way)
		\item actions
		\item plans	(imperative way)
		\item events
		\item three different kind of rules
			\begin{itemize}
				\item generate plans for achieving goals
				\item process events and received messages
				\item handle and repair failed plans
			\end{itemize}			
	\end{itemize}	
...
\begin{frame}[fragile,allowframebreaks]
	...
	\begin{itemize}
		...
		\item The specification of belief update actions do not cahnge during agent execution
	\end{itemize}
\end{frame}
...
\frame[allowframebreaks]{
	...
	The exepcetion are used to apply plan repair rules. They contain the identifier of the failed plan such that it can be determined which plan needs to be repaired.
}

\frame[allowframebreaks]{
	\frametitle{Including 2APL files}
Different agents may share certain initial beliefs, goals, plans, belief updates, and practical
reasoning rules. The 2APL programming language provides an encapsulation mechanism to
include these shared initial ingredients in different 2APL programs. This is done by allowing a
2APL program to be included in other 2APL programs through the Include: filename
construct.
...
\frame{
  \frametitle{Goal dynamics action (1/2)}
...
\frame{
  \frametitle{Goal dynamics action (2/2)}
...
\frame{
  ...
  Agents can communicate with each other by sending messages to each other. An
  agent can send a message to another agent by means of the
...
\frame[allowframebreaks]{
  ...
  maximum number of actions of the plan. Let
  $[\alpha_{1} ; \ldots ; \alpha_{n} ]$ be an atomic plan.
...
\frame{
  ...
  their constituent actions. This idea is captured by the following transition
  rule which states that an agent executes one of its plans at each computation
  step.
...
\frame{
  ...
  A planning goal rule is of the form:
...
\frame{
  ...
  A procedural Rule, is of the form:
